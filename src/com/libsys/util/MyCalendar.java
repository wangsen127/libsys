...
/**
 * //注册日历控件
 *	MyCalendar.getInstance().register(jtname);
 *	//设置不可编辑
...
public class MyCalendar extends JPanel {
	...
	private MyCalendar(Date date, String format, int startDAY) {
	...
	public static MyCalendar getInstance(Date date, String format) {
		return new MyCalendar(date, format, defaultStartDAY);
	}

	public static MyCalendar getInstance(Date date) {
		return getInstance(date, DEFAULTFORMAT);
	}

	public static MyCalendar getInstance(String format) {
	...
	private void initCalendarPanel() {
		...
		calendarPanel.setBorder(BorderFactory
				.createLineBorder(new Color(0xAA, 0xAA, 0xAA)));
	...
	private class TitlePanel extends JPanel {
		...
		private void initTitlePanel() {
			...
			preYear.setBorder(BorderFactory.createEmptyBorder(2,
					10, 0, 0));
			preMonth.setBorder(BorderFactory.createEmptyBorder(2,
					15, 0, 0));
			nextMonth.setBorder(BorderFactory.createEmptyBorder(2,
					0, 0, 15));
			nextYear.setBorder(BorderFactory.createEmptyBorder(2,
					0, 0, 10));
	...
	private class DayLabel extends JLabel implements
			Comparator<DayLabel>, MouseListener,
			MouseMotionListener {
	...
	private class LabelManager {
		...
		public LabelManager() {
			list = new ArrayList<MyCalendar.DayLabel>();
		}
